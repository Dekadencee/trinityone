    ...
    _isFullXP(false), _maxLevel(0), _isBattleGround(isBattleGround), _isPvP(false)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER || IS_PLAYER_GUID(victim->GetCharmerOrOwnerGUID()))
        _isPvP = true;
...
void Player::InitStatsForLevel(bool reapplyMods)
{
    ...
    for (uint8 i = 0; i < 3; ++i)
// - research for 2.4.3        SetUInt32Value(PLAYER_NO_REAGENT_COST_1 + i, 0);
...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    else if (CharacterNameData const* nameData = sWorld->GetCharacterNameData(guid))    // To avoid a query, we select loaded data. If it doesn't exist, return.
    {
        ...
        uint32 charDelete_minLvl = CONFIG_CHARDELETE_MIN_LEVEL;
...
void Player::RewardReputation(Unit* victim, float rate)
{
    ...
    {
        int32 donerep1 = CalculateReputationGain(REPUTATION_SOURCE_KILL, victim->getLevel(), Rep->RepValue1, Rep->RepFaction1);
    ...
    {
        int32 donerep2 = CalculateReputationGain(REPUTATION_SOURCE_KILL, victim->getLevel(), Rep->RepValue2, Rep->RepFaction2);
...
// - not in 2.4.3 UGH more research on PvP           ApplyModUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS, 1, true);
        }
        else
        {
...
void Player::SetVisibleItemSlot(uint8 slot, Item* pItem) // Needs rewrite for 2.4.3
{
    if (pItem)
    {
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    // This should be fixed when quest XP is rewritten
...
// - not in 2.4.3. Lot of work to do on player loading    SetUInt64Value(PLAYER_FIELD_KNOWN_CURRENCIES, fields[47].GetUInt64());
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
        ...
        for (uint32 i = 0; i < EQUIPMENT_SLOT_END * 2; ++i)
            ss << GetUInt32Value(PLAYER_VISIBLE_ITEM_1_0 + i) << ' '; // Check update field for 2.4.3
...
/*
    ...
    if (spellInfo->SpellFamilyFlags  & noReagentMask)
        return true;
*/ // How did reagents work in 2.4.3
    return false;
}
...
// - Not needed in 2.4.3    RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_LANDING); // No fly zone - Parachute
}
...
*/ // Needs to be completely reqritten for 2.4.3
